...
/**
 * @file
 *   Calls translations.js
 */

// Add modal to the body.
chrometwo_require(['jquery'], function($) {
    // Append the modal to the body so that it can be shown later.
    $(function () {
        $('.modal').appendTo('body');
    });

    // Listen and navigate to a new Machine Translation when one is generated.
    $( document ).ajaxComplete(function() {
        // Redirect logic for after a MT is created and the modal is displayed.
        if ($('.modal-footer a').hasClass('view-translation-modal')) {
            // Redirect the user automatically to the newly created MT.
            window.location.replace($('.view-translation-modal').attr("href"));
        }
        // Close feedback modal after successful submission.
        if ($('.modal-body .alert').hasClass('alert-success')) {
    ...
    // Override EN page title and set it to the data from the machine translation.
    var machine_translated_title = $('.header .title').text();
    document.title = machine_translated_title;
